...
Loader.prototype.loadFixture = function (fixture, models) {
  this.logger.debug(
    `Loading fixture for model '${fixture.model}:'`,
    fixture.data
  );
  ...
  if (typeof fixture !== "object")
    throw new Error("expected fixture to be object, is " + typeof fixture);
  ...
  if (!Model) {
    ...
    typeof Model.prototype !== "object"
  ) {
    throw new Error(
      "models." +
        fixture.model +
        " appears to be a function. Perhaps you are importing model factory function? You should then use sequelize.import to create your model, see https://github.com/sequelize/express-example/blob/master/models/index.js#L17"
    );
  } else if (!Model.findOne) {
    throw new Error("models." + fixture.model + " is not a sequelize model.");
  }

  return this.prepFixtureData(fixture.data, Model).spread(function (
    data,
    many2many
  ) {
    var setManyToMany = function (instance) {
      ...
      if (Object.keys(many2many).length) {
        ...
        Object.keys(many2many).forEach(function (key) {
          var assoc = Model.modelDefinition.associations[key];
          ...
          many2many[key].forEach(function (relinst) {
            ...
            promises.push(
              instance[assoc.accessors.add](relinst.instance, options)
            );
          });
        });
      }
    ...
    var where = {};
    for (const k of Model.modelDefinition.attributes.keys()) {
      var fieldType = Model.modelDefinition.attributes
        .get(k)
        .type.getDataTypeId();
      if (
        data.hasOwnProperty(k) &&
        (!fixture.keys || fixture.keys.indexOf(k) !== -1) &&
        fieldType !== "GEOMETRY" &&
        fieldType !== "GEOGRAPHY" &&
        fieldType !== "VIRTUAL"
      ) {
        ...
        if (fieldType === "JSONB") {
        ...
        } else if (Model.modelDefinition.attributes.get(k).set && !ignoreSet) {
          var val = null;
          ...
          try {
            Model.modelDefinition.attributes.get(k).set.call(Model, data[k]);
            // don't include VIRTUAL column in filter condition
            where[k] = val;
          } catch (err) {
            throw new Error(
              "Error using Model.set method for model " +
                fixture.model +
                " property " +
                k +
                " (possibly due to the use of instance methods). You should use option ignoreSet: true.\n" +
                err
            );
          }
        } else {
    ...
    return Model.findOne(findOptions).then(function (instance) {
      ...
      return Model.build(data, buildOptions)
        .save(saveOptions)
...
Loader.prototype.prepFixtureData = function (data, Model) {
  ...
  Object.keys(data).forEach(function (key) {
    ...
    Object.keys(Model.modelDefinition.associations).forEach(function (
      assoc_name
    ) {
      if (
        Model.modelDefinition.associations[assoc_name].options &&
        (Model.modelDefinition.associations[assoc_name].options.through ==
          key ||
          Model.modelDefinition.associations[assoc_name].options.as == key)
      ) {
        return (foundInThroughName = assoc_name);
      }
    });

    var assoc = Model.modelDefinition.associations[foundInThroughName || key],
      val = data[key];
    ...
    if (assoc) {
      if (assoc.associationType === "BelongsTo") {
        ...
        var options =
          typeof val === "object" ? { where: val } : { where: where };
        if (self.options.transaction)
          options.transaction = self.options.transaction;
        promises.push(
          assoc.target.findOne(options).then(function (obj) {
            result[assoc.identifier] =
              obj[assoc.target.primaryKeyField || "id"];
        ...
        if (Array.isArray(val)) {
          ...
          throw new Error(
            "HasMany associations must be arrays of where clauses"
          );
        }
      } else {
      ...
      if (Model.modelDefinition.attributes.has(key)) {
        result[key] = val;
      } else {
        self.logger.warn(
          'attribute "' + key + "' not defined on  model '" + Model.name + "'."
        );
      }
    }
  });
