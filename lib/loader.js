...
Loader.prototype.loadFixture = function(fixture, models) {
    ...
    return this.prepFixtureData(fixture.data, Model).spread(function(data, many2many) {
        ...
        var hasExplicitPrimaryKeys = false;
        for (var primaryKey in Model.primaryKeys) {
            if (!Model.primaryKeys[primaryKey]._autoGenerated) {
                hasExplicitPrimaryKeys = true;
                break;
            }
        }

        var toWhereClauseParameter = function(fieldType, whereValue, rawAttributes) {
            // postgres
            if (fieldType === 'JSONB') {
                return {
                    $contains: whereValue
                };
            } else if (rawAttributes.hasOwnProperty('set')) {
                var val = null;
                ...
                rawAttributes.set.call(Model, whereValue);
        ...
        Object.keys(Model.rawAttributes).forEach(function(k) {
            ...
            if (data.hasOwnProperty(k) && (!fixture.keys || fixture.keys.indexOf(k) !== -1) && fieldType !== 'GEOMETRY' &&
                fieldType !== 'VIRTUAL' && (!hasExplicitPrimaryKeys || isPrimaryKey)) {
                where[k] = toWhereClauseParameter(fieldType, data[k], Model.rawAttributes[k]);
            }
        });
