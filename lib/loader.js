    ...
    Object.keys(data).forEach(function(key) {
        ...
        if (assoc) {
            if (assoc.associationType === 'BelongsTo') {
                promises.push(
                    (typeof val === 'object' ?  assoc.target.find({where: val}) : assoc.target.findOne({ where: { id: val } }))
                    .then(function(obj) {
                ...
                if (Array.isArray(val)) {
                    ...
                    val.forEach(function(v) {
                        promises.push(
                            (typeof v === 'object' ?  assoc.target.find({where: v}) : assoc.target.findOne({ where: { id: v } }))
                            .then(function(obj) {
