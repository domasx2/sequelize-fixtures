            ...
            if (data.hasOwnProperty(k) && (!fixture.keys || fixture.keys.indexOf(k) !== -1) && fieldType !== 'GEOMETRY' && fieldType !== 'VIRTUAL') {
                ...
                if (fieldType === 'JSONB') {
                    try {
                        ...
                        // Throw an error if a deep dive into the object finds an array
                        (function A(ob) {
                            if (typeof ob === 'object') {
                                if (_.isArray(ob)) {
                                    throw 'Array!';
                        ...
                        //console.log(e);
                        // Do nothing because of a sequelize bug
    ...
    Object.keys(data).forEach(function(key) {
        ...
        if (assoc) {
            if (assoc.associationType === 'BelongsTo') {
                ...
                promises.push(
                    (typeof val === 'object' ?  assoc.target.findOne(options) : assoc.target.findOne(options))
                    .then(function(obj) {
                ...
                if (Array.isArray(val)) {
                    ...
                    val.forEach(function(v) {
                        ...
                        promises.push(
                            (typeof v === 'object' ?  assoc.target.findOne(options) : assoc.target.findOne(options))
                            .then(function(obj) {
