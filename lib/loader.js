            ...
            if (data.hasOwnProperty(k) && (!fixture.keys || fixture.keys.indexOf(k) !== -1) && fieldType !== 'GEOMETRY' && fieldType !== 'VIRTUAL') {
                ...
                if (fieldType === 'JSONB') {
                    ...
                    try {
                      ...
                      console.warn('Error using Model.set method for model ' + fixture.model+ ' property ' + k + ' (possibly due to the use of instance methods). Falling back to raw data assignment.');
                      where[k] = data[k];
                    }
                } else {
                    where[k] = data[k];
